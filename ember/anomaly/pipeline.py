...
from ember.anomaly.embeddings import MultiModelExtractor, prepare_multimodel_spectrograms
from ember.anomaly.ensemble import compute_default_ensembles, score_lightgbm_meta_learner
...
def build_feature_bank(
    ...
    *,
    ...
    physics_features = extract_physics_feature_matrix(dataset.specs_raw, n_bands=n_bands)
    ...
    if resolved_channel is not None:
        paired_specs = []
        for primary, raw_aux in zip(dataset.specs_raw, dataset.df[resolved_channel].tolist()):
            try:
        ...
        coupling_features = extract_coupling_feature_matrix(dataset.specs_raw, paired_specs)
...
def run_classical_anomaly_workflow(
    ...
    *,
    ...
    if strict_evaluation:
        ...
        ae_scores, ae_latents = score_with_ae(ae_model, dataset.specs_raw, ae_mu, ae_std)
...
def run_robust_anomaly_pipeline(
    ...
    *,
    ...
    if verbose:
        print(f"bg_train={len(bg_train_idx)}  bg_cal={len(bg_cal_idx)}"
    ...
    if verbose:
        ...
        print(f"    TPR={tpr_c:.1%}  FAR_all={far_c_all:.2%} ({fp_c_all}/{n_all_bg_e} bg)"
              f"  FAR_cal={far_c_cal:.2%} ({fp_c_cal}/{n_bg_cal_e} bg_cal)")
        print(f"  1%-FAR threshold (budget vs all {n_all_bg_e} bg):")
        print(f"    TPR={tpr_1:.1%}  FAR_all={far_1_all:.2%} ({fp_1_all}/{n_all_bg_e} bg)"
    ...
    if output_dir is not None:
        ...
        try:
            ...
            fig_dist.savefig(out / "score_distributions.png", dpi=120, bbox_inches="tight")
            ...
            fig_roc = plot_roc_with_operating_points(
                ...
                operating_points=[
                    {"fpr": far_c_all, "tpr": tpr_c,
                     "label": f"Conservative ({far_c_all:.1%} FAR)"},
                    {"fpr": far_1_all, "tpr": tpr_1,
